 ...
 * concrete manager.
 *
 * Modifying add email_or_phone field
 *
 * @author by Vinh Banh <apacheservices68@gmail.com>
 */
abstract class UserManager implements UserManagerInterface
{
    ...
    /**
     * @param $emailOrPhone
     * @return \FOS\UserBundle\Model\UserInterface|mixed|null
     */
    public function findUserByEmailOrPhone($emailOrPhone){
        if (preg_match('/^.+\@\S+\.\S+$/', $emailOrPhone)) {
            $user = $this->findUserByEmail($emailOrPhone);
            if (null !== $user) {
                return $user;
            }
        }

        return $this->findUserBy(array('emailOrPhone' => $emailOrPhone));
    }
